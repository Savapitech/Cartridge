...
uint8_t menu(bank_t *player_bank)
{
    ...
    while (!(keys & J_A)) 
    {
        ...
        if (game_idx != last_idx) {
            ...
            draw_text(10 - (game_tab[up_idx].name_size >> 1),   MENU_UP_TXT,   game_tab[up_idx].name);
            draw_text(10 - (game_tab[game_idx].name_size >> 1), MENU_MDL_TXT,  game_tab[game_idx].name);
            draw_text(10 - (game_tab[game_idx].name_size >> 1) - 3, MENU_MDL_TXT,  ARROW);
            draw_text(10 - (game_tab[down_idx].name_size >> 1), MENU_DOWN_TXT, game_tab[down_idx].name);
